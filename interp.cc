...
__attribute__((noreturn)) static void signal_error(const char* message, const char *what) {
  if (what)
...
public:
  ...
  std::string toString() const {
    return "(func " + n2hexstr(reinterpret_cast<uintptr_t>(jitCode)) + " " + body->toString() + ")";
  }
...
public:
  ...
  std::string toString() const {
    return "(var " + std::to_string(depth) + ")";
  }
...
public:
  ...
  std::string toString() const {
    return "(" + opToString(op) + " " + lhs->toString() + " " + rhs->toString() + ")";
  }
...
public:
  ...
  std::string toString() const {
    return "(if " + test->toString() + " " + consequent->toString() + " " + alternate->toString() + ")";
  }
